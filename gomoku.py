class Juego:
    ...
    def colocarPieza(self, coord, turno):
        if(coord.f, coord.c) in self.libres:
            self.libres.remove((coord.f, coord.c))
        ...
        return
    def EmularcolocarPieza(self, coord, turno):
        if(coord.f, coord.c) in self.libres:
            self.libres.remove((coord.f, coord.c))
        "Coloca en la posicion coord el valor turno"
        self.tablero[coord.f][coord.c] = turno;
        ...
        for i in range(15):
            for j in range(15):
                ...
                if(j==14):
                    str1 += "\n"
        print(str1)
    def determinarMejorMovimiento(self,turno):
        max_valor = 0
        for (x, y) in self.libres:
           Juego.colocarPieza(Coord(x,y),turno)
        return
...
juego = Juego()
print(len(juego.libres))

juego.colocarPieza(Coord(2, 5), 1)
